...
// ── Specialisation bonus derives ───────────────────────────────

#[test]
fn spec_hit_bonus_single_is_1() {
    ...
    let val = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_hit_bonus",
            vec![enum_variant("SpecLevel", "Single")],
        )
        .unwrap();
...
fn spec_hit_bonus_double_is_3() {
    ...
    let val = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_hit_bonus",
            vec![enum_variant("SpecLevel", "Double")],
        )
        .unwrap();
...
#[test]
fn spec_damage_bonus_single_is_2() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;
...
#[test]
fn spec_damage_bonus_double_is_3() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;
...
#[test]
fn can_specialize_fighter_types_only() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;

    let eligible = ["Fighter", "Paladin", "Ranger"];
    let ineligible = [
        "Cleric",
        "MagicUser",
        "Thief",
    ...
    for class in &eligible {
        let val = interp
            .evaluate_derive(
                ...
                &mut handler,
                "can_specialize",
                vec![enum_variant("Class", class)],
            )
            .unwrap();
        assert_eq!(val, Value::Bool(true), "{class} should be eligible");
    }
    ...
    for class in &ineligible {
        let val = interp
            ...
            .unwrap();
        assert_eq!(val, Value::Bool(false), "{class} should not be eligible");
    }
}
...
#[test]
fn double_spec_excludes_polearm_and_two_handed() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;
    ...
    for weapon in &["PoleArm", "SwordTwoHanded"] {
        let val = interp
            .evaluate_derive(
                &state,
                &mut handler,
                "can_double_specialize_melee",
                vec![enum_variant("MeleeWeapon", weapon)],
            )
            .unwrap();
        assert_eq!(val, Value::Bool(false), "{weapon} should not double-spec");
    }

    // Other weapons can
    for weapon in &["SwordLong", "Dagger", "BattleAxe", "MaceHeavy"] {
        let val = interp
            .evaluate_derive(
                &state,
                &mut handler,
            ...
            .unwrap();
        assert_eq!(val, Value::Bool(true), "{weapon} should double-spec");
    }
}
...
#[test]
fn spec_melee_attacks_low_level() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;

    // L1-6: 3/2 → alternating 2/1
    let round1 = interp
        .evaluate_derive(
            &state,
            &mut handler,
    ...
    let round2 = interp
        ...
        .unwrap();
    assert_eq!(round1, Value::Int(1), "L5 odd round: 1 attack");
    assert_eq!(round2, Value::Int(2), "L5 even round: 2 attacks");
}

#[test]
fn spec_melee_attacks_mid_level() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;
    ...
    for round in 1..=4 {
        let val = interp
            .evaluate_derive(
                ...
                &mut handler,
                "spec_melee_attacks",
                vec![Value::Int(10), Value::Int(round)],
            )
            .unwrap();
        assert_eq!(val, Value::Int(2), "L10 round {round}: always 2 attacks");
    }
}

#[test]
fn spec_melee_attacks_high_level() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;

    // L13+: 5/2 → alternating 3/2
    let round1 = interp
        .evaluate_derive(
            &state,
            &mut handler,
    ...
    let round2 = interp
        ...
        .unwrap();
    assert_eq!(round1, Value::Int(2), "L15 odd round: 2 attacks");
    assert_eq!(round2, Value::Int(3), "L15 even round: 3 attacks");
}

// ── Specialised missile rate of fire ───────────────────────────

#[test]
fn spec_missile_rof_pulled_bow() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;

    let bow = || enum_variant("MissileWeapon", "BowLong");
    ...
    let val = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![bow(), Value::Int(3), Value::Int(1)],
        )
        .unwrap();
    assert_eq!(val, Value::Int(2));
    ...
    let val = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![bow(), Value::Int(10), Value::Int(1)],
        )
        .unwrap();
    assert_eq!(val, Value::Int(3));
    ...
    let val = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![bow(), Value::Int(15), Value::Int(1)],
        )
        .unwrap();
...
#[test]
fn spec_missile_rof_heavy_crossbow() {
    let (program, result) = compile_osric_weapon_spec();
    let interp = Interpreter::new(&program, &result.env).unwrap();
    let state = GameState::new();
    let mut handler = NullHandler;

    let xbow = || enum_variant("MissileWeapon", "CrossbowHeavy");

    // L1-6: every other round (1 odd, 0 even)
    let odd = interp
        .evaluate_derive(
            &state,
            &mut handler,
            "spec_missile_rof",
            vec![xbow(), Value::Int(3), Value::Int(1)],
        )
        .unwrap();
    let even = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![xbow(), Value::Int(3), Value::Int(2)],
        )
        .unwrap();
    assert_eq!(odd, Value::Int(1), "heavy xbow L3 odd: 1");
    assert_eq!(even, Value::Int(0), "heavy xbow L3 even: 0");
    ...
    let val = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![xbow(), Value::Int(10), Value::Int(1)],
        )
        .unwrap();
    assert_eq!(val, Value::Int(1));
    ...
    let odd = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![xbow(), Value::Int(15), Value::Int(1)],
        )
        .unwrap();
    let even = interp
        .evaluate_derive(
            ...
            &mut handler,
            "spec_missile_rof",
            vec![xbow(), Value::Int(15), Value::Int(2)],
        )
        .unwrap();
    assert_eq!(odd, Value::Int(2), "heavy xbow L15 odd: 2");
    assert_eq!(even, Value::Int(1), "heavy xbow L15 even: 1");
}
