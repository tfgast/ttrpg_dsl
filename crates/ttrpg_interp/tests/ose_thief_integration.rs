...
fn ose_thief_parses_and_typechecks() {
    ...
    let ose_count = program.items.iter().filter(|item| {
        matches!(&item.node, TopLevel::System(sys) if sys.name == "OSE")
    }).count();
    assert!(ose_count >= 2, "expected at least 2 system 'OSE' blocks, got {}", ose_count);
}
...
fn ose_thief_has_expected_declarations() {
    ...
    let tables: Vec<_> = decls.iter().filter_map(|d| match &d.node {
        DeclKind::Table(t) => Some(&*t.name),
        _ => None,
    }).collect();
    assert!(tables.contains(&"skill_chance"), "missing table skill_chance");

    let mechanics: Vec<_> = decls.iter().filter_map(|d| match &d.node {
    ...
    assert!(derives.contains(&"backstab_multiplier"), "missing derive backstab_multiplier");
    assert!(derives.contains(&"has_thief_skills"), "missing derive has_thief_skills");
    assert!(derives.contains(&"can_backstab"), "missing derive can_backstab");
}
...
fn skill_chance_climb_walls() {
    ...
    let val = interp.evaluate_derive(
        &state, &mut handler, "test_skill_chance",
...
fn backstab_multiplier_by_level() {
    ...
    // Level 1 → ×2
    let val = interp.evaluate_derive(
        &state, &mut handler, "backstab_multiplier",
        vec![Value::Int(1)],
    ).unwrap();
    assert_eq!(val, Value::Int(2));
...
fn has_thief_skills_class_check() {
    ...
    // Thief → true
    let val = interp.evaluate_derive(
        &state, &mut handler, "has_thief_skills",
...
fn can_backstab_class_check() {
    ...
    // Thief → true
    let val = interp.evaluate_derive(
        &state, &mut handler, "can_backstab",
        vec![enum_variant("Class", "Thief")],
    ).unwrap();
    assert_eq!(val, Value::Bool(true));
...
fn thief_skill_check_success() {
    ...
    let val = interp.evaluate_mechanic(
        &state, &mut handler, "thief_skill_check",
